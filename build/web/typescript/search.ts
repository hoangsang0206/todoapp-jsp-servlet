...
const appendSearchCategories = (data) => {
    ...
    data.map(item => {
        const str: string = `<a href="./category?cid=${item.id}" class="search-cate-item d-flex align-items-center gap-1 text-decoration-none">
...
const appendSearchTodoList = (data) => {
    $('.search-todo').empty();
    data.map(item => {
        const str: string = `<div class="horizon-task-box d-flex align-items-center justify-content-between gap-2">
